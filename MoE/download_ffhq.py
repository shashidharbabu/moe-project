...
def download_file(session, file_spec, stats, chunk_size=128, num_attempts=10, **kwargs):
    ...
    for attempts_left in reversed(range(num_attempts)):
        ...
        try:
            ...
            # Handle known failure cases (including larger HTML responses).
            if data_size > 0:
                try:
                    with open(tmp_path, 'rb') as f:
                        data = f.read(min(1_000_000, data_size))  # read up to 1MB to inspect
                    data_str = data.decode('utf-8', errors='ignore')
                except Exception:
                    data_str = ''

                lowercase_html = data_str.lower()

                # Google Drive virus checker nag (export=download link)
                if 'export=download' in lowercase_html or 'export=download' in data_str:
                    links = [html.unescape(link) for link in data_str.split('"') if 'export=download' in link]
                    if len(links) == 1 and attempts_left:
                        file_url = requests.compat.urljoin(file_url, links[0])
                        continue

                # Google Drive quota exceeded page
                if 'quota exceeded' in lowercase_html or 'google drive - quota exceeded' in data_str:
                    if not attempts_left:
                        raise IOError("Google Drive download quota exceeded -- please try again later")
                    # If attempts remain, try again after short delay
                    time.sleep(1.0)
                    continue
